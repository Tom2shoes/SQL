...
SELECT last_name, COUNT(Last_name) as '# of repetitions'
FROM actor
GROUP BY last_name;
...
SELECT f.title, COUNT(i.film_id) as 'total inventory'
FROM inventory AS i
...
SELECT f.title, i.film_id, COUNT(r.rental_date)
FROM film f
...
SELECT genre.name, SUM(p.amount)
FROM category AS genre
INNER JOIN film_category fc ON fc.category_id = genre.category_id
